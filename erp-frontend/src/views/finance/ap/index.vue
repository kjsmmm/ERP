<template>
  <div>
    ...
    <div class="search-bar">
      <el-input v-model="keyword" placeholder="搜索应付单号" clearable style="width: 200px" @clear="loadData" @keyup.enter="loadData" />
    ...
    <el-table :data="tableData" v-loading="loading" stripe>
      <el-table-column prop="payableNo" label="应付单号" width="160" />
      ...
      <el-table-column label="状态" width="100">
        <template #default="{ row }">
          <el-tag :type="apStatusType(row.status)">{{ apStatusText(row.status) }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" width="200">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status !== 2" link type="success" @click="handlePayment(row)">付款</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="invoiceVisible" title="登记发票" width="400px">
      <el-form :model="invoiceForm" ref="invoiceFormRef" label-width="80px">
        ...
        <el-form-item label="来票日期" prop="invoiceDate">
    ...
    <el-dialog v-model="paymentVisible" title="付款核销" width="400px">
...
import { getApRecordPage, updateInvoice, addPayment } from '@/api/finance/apRecord';
import type { ApRecord } from '@/types/finance';
import { AP_STATUS_MAP } from '@/types/finance';

const loading = ref(false);
const tableData = ref<ApRecord[]>([]);
...
async function loadData() {
  loading.value = true;
  try {
    const res = await getApRecordPage({ keyword: keyword.value, status: statusFilter.value, pageNum: pageNum.value, pageSize: pageSize.value });
...
function handleInvoice(row: ApRecord) {
  currentApId.value = row.id;
...
async function handleInvoiceSubmit() {
  ...
  await updateInvoice(currentApId.value, { ...invoiceForm });
  ElMessage.success('发票登记成功');
  invoiceVisible.value = false;
  loadData();
}

function handlePayment(row: ApRecord) {
  currentApId.value = row.id;
...
async function handlePaymentSubmit() {
  ...
  await addPayment(currentApId.value, { ...paymentForm });
  ElMessage.success('付款成功');
  paymentVisible.value = false;
  loadData();
}

function apStatusText(status?: number) {
  if (status === undefined) return '';
  return AP_STATUS_MAP[status]?.label || '未知';
}

function apStatusType(status?: number) {
  if (status === undefined) return 'info';
  return (AP_STATUS_MAP[status]?.type || 'info') as 'info' | 'warning' | 'success' | 'danger' | '';
}
</script>
